...
func countAndSay(n int) string {
	...
	for i := 1; i < len(previous); i++ {
		if previous[i] == previous[i-1] {
			...
			result += strconv.Itoa(count) + string(previous[i-1])
			count = 1
		}
	}

	// Add the last sequence
	result += strconv.Itoa(count) + string(previous[len(previous)-1])
...
func main() {
	...
	fmt.Println(countAndSay(2)) // Output: "11"
	fmt.Println(countAndSay(3)) // Output: "21"
