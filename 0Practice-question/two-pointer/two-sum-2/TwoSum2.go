...
func main() {
	arr := []int{-1, 0}
	answer := twoSum2(arr, -1)
	fmt.Println(answer)
}

func twoSum2(arr []int, target int) []int {
	left, right := 0, len(arr)-1
	// searching each element from left to right
	for left < right {
		sum := arr[left] + arr[right]
		if sum == target {
			//if sum is found then return the sum
			return []int{left + 1, right + 1}
		} else if sum < target {
			//if sum is less add grater value array is sorted then left++
			left++
		} else {
			//is is grater so right --
			right--
		}
	}
	//no sum found result empty array
	return []int{}
}
